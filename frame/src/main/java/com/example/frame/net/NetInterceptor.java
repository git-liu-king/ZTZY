...
import com.example.frame.utils.AES;
...
import static com.example.frame.net.NetworkUtils.isNetworkConnected;
...
public class NetInterceptor {
    ...
    static class CommonParamsInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            HttpUrl httpUrl = request.url()
                    ...
                    .addQueryParameter("token1", BaseApp.getApplication().mToken)
    ...
    private static void saveSession(Response response) {
        ...
        if (!TextUtils.isEmpty(session)) {
            BaseApp.mSeesion = session;
            SharedPrefrenceUtils.saveString(BaseApp.getContext(), "session", session);
        }
    }
