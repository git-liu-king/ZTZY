...
public class NetHeaders {
    ...
    public static Map getHeadMap() {
        ...
        // headers.put("Authorization", BaseApp.getApplication().mToken);
        headers.put("UUID", BaseApp.getApplication().mUuid.toString());
        headers.put("device_id", BaseApp.getApplication().mUuid.toString());
        ...
        headers.put("device-tag", "0");
        headers.put("Cookie", BaseApp.mSeesion);
...
//        header.put("sign", sign);
    ...
    public static String getUserAgent() {
        int versionCode = 1;
        try {
            versionCode = BaseApp.getApplication().getPackageManager().getPackageInfo(BaseApp.getApplication().getPackageName(), 0).versionCode;
        } catch (PackageManager.NameNotFoundException pE) {
            pE.printStackTrace();
        }
        String s = WebSettings.getDefaultUserAgent(BaseApp.getContext()) + " News/" + versionCode + " Android/"
